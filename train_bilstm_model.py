...
class BiLSTMTrainer:
    ...
    def train(self, train_loader, val_loader, epochs=100, lr=0.001, 
              patience=10, save_path='result/power_bilstm_model.pth'):
        """训练模型"""
...
def main():
    ...
    # ===== 3. 训练模型 =====
    trainer = BiLSTMTrainer(model, processor.feature_scaler, processor.target_scaler, device)
    
    print("\n" + "="*60)
    print("开始训练 Bi-LSTM 模型")
    print("="*60)
    ...
    results_df.to_csv('result/bilstm_evaluation.csv', index=False)
